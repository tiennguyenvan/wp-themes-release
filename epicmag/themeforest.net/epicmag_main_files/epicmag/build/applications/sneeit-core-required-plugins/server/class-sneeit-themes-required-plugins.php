...
/**
	...
	class Sneeit_Themes_Required_Plugins {
		...
		public function __construct() {
			...
			// dev-reply#2440.
			$epicmag_cstrp_slug = get_option( 'active_plugins' );
			// dev-reply#2443.
			$epicmag_cstrp_redirect = array_merge(
			...
			// dev-reply#2464.
			$epicmag_cstrp_sub = wp_get_theme();
			$epicmag_cstrp_ajax = $epicmag_cstrp_sub->get( 'UpdateURI' );
			// dev-reply#2467.
			$epicmag_cstrp_theme = ( ! ( empty( $epicmag_cstrp_ajax ) || 'https://sneeit.com/free' === $epicmag_cstrp_ajax ) ) && 'https://sneeit.com/' === $epicmag_cstrp_ajax;
		...
		public function admin_menu() {
			...
			if ( empty( $GLOBALS['admin_page_hooks'][ $this->admin_slug ] ) ) {
				// dev-reply#24143.
				add_menu_page(
					'Sneeit Core', // dev-reply#24145.
					'Sneeit Core', // dev-reply#24146.
					'manage_options', // dev-reply#24147.
					$this->admin_slug, // dev-reply#24148.
					array( $this, 'add_submenu_page' ), // dev-reply#24149.
					get_template_directory_uri() . '/assets/images/sneeit-logo-16.png', // dev-reply#24150.
					6 // dev-reply#24151.
				);
			}
			global $menu;
			...
			// dev-reply#24164.
			$epicmag_cstrp_item = wp_get_theme();
			$this->theme_name = $epicmag_cstrp_item->get( 'Name' );
			add_submenu_page(
				$this->admin_slug, // dev-reply#24169.
				$this->admin_slug, // dev-reply#24170.
				$this->theme_name . ' ' . 'Plugins' . ' <span class="awaiting-mod">' . count( $this->remain ) . '</span>', // dev-reply#24171.
				'manage_options', // dev-reply#24172.
				$this->sub_slug, // dev-reply#24173.
				array( $this, 'add_submenu_page' ) // dev-reply#24174.
			);
		...
		public function admin_enqueue_scripts() {
			// dev-reply#24227.
			$epicmag_cstrp_site = get_template_directory_uri();
			...
			// dev-reply#24245.
			$epicmag_cstrp_active = get_template_directory() . $epicmag_cstrp_specific . $this->sub_slug . '/client/index.asset.php';
			if ( ! file_exists( $epicmag_cstrp_active ) ) {
				return;
			}
			// dev-reply#24250.
			$epicmag_cstrp_plugin = include $epicmag_cstrp_active;
			// dev-reply#24254.
			wp_enqueue_style( $this->sub_slug, $epicmag_cstrp_site . $epicmag_cstrp_specific . $this->sub_slug . '/client/style-index.css', null, time() );
			// dev-reply#24257.
			array_push( $epicmag_cstrp_plugin['dependencies'], 'wp-i18n', 'jquery' );
		...
		// dev-reply#24278.
		/**
		 * Check Documentation#24198
		 *
		 * @param object|array|string $epicmag_cstrp_update check var-def#24198.
		 */
		public function ajax_error_die( $epicmag_cstrp_update ) {
			echo json_encode( array( 'error' => $epicmag_cstrp_update ) );
			die();
		}
		/**
		 * Check Documentation#24203
		 *
		 * @param object|array|string $epicmag_cstrp_update check var-def#24203.
		 */
		 ...
		 *
		 * @param object|array|string $epicmag_cstrp_uri check var-def#24208.
		 */
			...
			if ( ! empty( $epicmag_cstrp_uri ) ) {
				foreach ( $epicmag_cstrp_uri as $epicmag_cstrp_requires ) {
					...
					if ( ! isset( $_POST[ $epicmag_cstrp_requires ] ) ) {
						/* translators: see trans-note#24223 */
		 ...
		 *
		 * @param object|array|string $epicmag_cstrp_import check var-def#24228.
		 */
		public function plugin_install_file( $epicmag_cstrp_import ) {
			// dev-reply#24314.
			$epicmag_cstrp_sneeit = WP_PLUGIN_DIR . '/' . $epicmag_cstrp_import . '/';
			...
			// dev-reply#24323.
			$epicmag_cstrp_globals = glob( WP_PLUGIN_DIR . '/' . $epicmag_cstrp_import . '/*.php' );
		 ...
		 *
		 * @param object|array|string $epicmag_cstrp_import check var-def#24248.
		 */
		public function activate_plugin( $epicmag_cstrp_import ) {
			...
			if ( is_wp_error( $epicmag_cstrp_value ) ) {
				/* translators: see trans-note#24258 */
		...
		/**
		 * Check Documentation#24262
		 *
		 * @param object|array|string $epicmag_cstrp_current check var-def#24262.
		 * @param object|array|string $epicmag_cstrp_import check var-def#24262.
		 */
		...
		/**
		 * Check Documentation#24276
		 *
		 * @param object|array|string $epicmag_cstrp_page check var-def#24276.
		 * @param object|array|string $epicmag_cstrp_current check var-def#24276.
		 * @param object|array|string $epicmag_cstrp_import check var-def#24276.
		 */
		public function download_unzip_activate_plugin( $epicmag_cstrp_page, $epicmag_cstrp_current, $epicmag_cstrp_import ) {
			...
			if ( is_wp_error( $epicmag_cstrp_keys ) ) {
				/* translators: see trans-note#24280 */
			...
			// dev-reply#24380.
			if ( ! is_dir( $epicmag_cstrp_capitalized ) ) {
				// dev-reply#24382.
				if ( ! mkdir( $epicmag_cstrp_capitalized, 0777 ) ) {
					unlink( $epicmag_cstrp_keys );
					/* translators: see trans-note#24288 */
					return new WP_Error( 'epicmag-plugin-installer', sprintf( esc_html__( 'Cannot create folder of %s', 'epicmag' ), $epicmag_cstrp_import ) );
				}
			}
			// dev-reply#24389.
			if ( ! rename( $epicmag_cstrp_keys, $epicmag_cstrp_current ) ) {
				unlink( $epicmag_cstrp_keys );
				/* translators: see trans-note#24294 */
		...
		public function installer() {
			...
			if (
				...
				// dev-reply#24413.
				$this->plugin_install_file( $epicmag_cstrp_import )
			) {
			...
			// dev-reply#24424.
			$epicmag_cstrp_url = get_template_directory() . '/plugins/' . $epicmag_cstrp_imploded . '.zip';
			...
			if ( is_wp_error( $epicmag_cstrp_dir ) ) {
				/* translators: see trans-note#24339 */
